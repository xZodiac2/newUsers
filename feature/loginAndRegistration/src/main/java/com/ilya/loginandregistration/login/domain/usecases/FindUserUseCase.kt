...
class FindUserUseCase @Inject constructor(
    ...
    override operator fun invoke(data: Any): Result<LoggedInUserData> {
        val loginParams = data as? UserLoginParams ?:
            return Result.failure(LoginDomainError.WrongLoginArgument)
        
        return usersRepository.searchByLogin(loginParams.login).fold(
            onSuccess = { userData ->
