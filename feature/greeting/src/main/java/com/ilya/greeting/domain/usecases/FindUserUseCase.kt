...
class FindUserUseCase @Inject constructor(
    ...
    override operator fun invoke(data: Any): Result<GreetingUserData> {
        return usersRepository.searchByLogin(data.toString()).fold(
            onSuccess = { Result.success(GreetingUserData(it.name)) },
            onFailure = { error ->
                when (error) {
                    is UsersDataError.NotFound -> Result.failure(GreetingDomainError.UserNotFound)
                    else -> Result.failure(GreetingDomainError.UnknownError)
                }
            }
        )
    }
